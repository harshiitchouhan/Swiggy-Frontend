...
return(
     ...
     <div >
          <p className="text-black font-bold text-base pl-1 pt-2 truncate">{swiggydata?.info?.name}</p>
          <p className="text-black font-semibold text-base pl-1 flex items-center">
               <span className="text-green-700 pr-1 text-xl pb-1">★</span>
               {swiggydata?.info?.avgRatingString} • {swiggydata?.info?.sla?.slaString}
          </p>
          <p className="text-gray-600 text-base pl-1 truncate">
               {swiggydata?.info?.cuisines?.join(", ")}
          </p>   
          <p className="text-gray-600 text-base pl-1 truncate">{swiggydata?.info?.areaName}</p>
     </div>
